...
export function PublicSpeakerModal({
  ...
  const filteredVisiting = useMemo(() => {
    ...
    if (speakerSearch) {
      ...
      list = list.filter(s => s.name.toLowerCase().includes(q) || s.congregation.toLowerCase().includes(q));
    }
    return list;
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-1/2 border-r border-gray-200 flex flex-col">
              ...
              {(['local', 'visiting', 'other'] as const).map(t => (
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-semibold capitalize border-b-2 transition-colors ${
                    speakerType === t ? 'border-sky-500 text-sky-700 bg-sky-50' : 'border-transparent text-gray-500 hover:bg-gray-50'
                  }`}
                >
                  ...
                  {localSpeakerList
                    ...
                    .map(s => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-xs border-b border-gray-100 ${
                          selectedLocalId === s.id ? 'bg-yellow-100 font-semibold' : 'hover:bg-sky-50'
                        }`}
                      >
                        {speakerName(s)}
                      </button>
                ...
                <div className="flex-1 overflow-y-auto border border-gray-200 rounded">
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        <th className="text-left px-2 py-1 font-semibold text-gray-600">Nombre</th>
                        <th className="text-left px-2 py-1 font-semibold text-gray-600">Congregación</th>
                        <th className="text-left px-2 py-1 font-semibold text-gray-600">Último</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredVisiting.map(s => (
                        <tr
                          ...
                          className={`cursor-pointer border-b border-gray-100 ${
                            selectedVisitingId === s.id ? 'bg-yellow-100' : 'hover:bg-sky-50'
                          }`}
                        >
                          <td className="px-2 py-1">{s.name}</td>
                          <td className="px-2 py-1 text-gray-500">{s.congregation}</td>
                          <td className="px-2 py-1 text-gray-400">{s.last_given_date?.slice(0, 10) ?? ''}</td>
                        </tr>
                      ))}
                      {filteredVisiting.length === 0 && (
                        <tr><td colSpan={3} className="px-2 py-4 text-center text-gray-400">Sin oradores visitantes</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            {speakerType === 'other' && (
              <div className="p-4 flex flex-col gap-2">
                <label className="text-xs text-gray-600">Nombre del orador</label>
                <input
                  type="text"
          ...
          <div className="w-1/2 flex flex-col">
            <div className="flex border-b border-gray-200">
              <button
                ...
                className={`flex-1 py-2 text-xs font-semibold border-b-2 transition-colors ${
                  outlineMode === 'standard' ? 'border-sky-500 text-sky-700 bg-sky-50' : 'border-transparent text-gray-500 hover:bg-gray-50'
                }`}
              >
                Discurso Estándar
              </button>
              <button
                ...
                className={`flex-1 py-2 text-xs font-semibold border-b-2 transition-colors ${
                  outlineMode === 'special' ? 'border-sky-500 text-sky-700 bg-sky-50' : 'border-transparent text-gray-500 hover:bg-gray-50'
                }`}
              >
                Especial
              </button>
              ...
              <div className="flex flex-col flex-1 overflow-hidden p-2 gap-2">
                ...
                <div className="flex-1 overflow-y-auto border border-gray-200 rounded">
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        <th className="text-left px-2 py-1 w-8 font-semibold text-gray-600">#</th>
                        <th className="text-left px-2 py-1 font-semibold text-gray-600">Título</th>
                        <th className="text-left px-2 py-1 w-24 font-semibold text-gray-600">Último</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredOutlines.map(o => {
                        ...
                        return (
                          <tr
                            ...
                            className={`cursor-pointer border-b border-gray-100 ${bg} hover:brightness-95`}
                          >
                            <td className="px-2 py-1 text-gray-500">{o.number}</td>
                      ...
                      {filteredOutlines.length === 0 && (
                        <tr><td colSpan={3} className="px-2 py-4 text-center text-gray-400">Sin discursos</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                <div className="flex gap-3 text-[10px] text-gray-500 pt-1">
                  <span className="flex items-center gap-1"><span className="w-3 h-3 bg-red-200 border border-gray-200 inline-block rounded-sm" />Reciente (&lt;3 meses)</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-3 bg-red-100 border border-gray-200 inline-block rounded-sm" />6 meses</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-3 bg-orange-50 border border-gray-200 inline-block rounded-sm" />1 año</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-3 bg-white border border-gray-200 inline-block rounded-sm" />Antiguo</span>
                </div>
              </div>
            )}
            ...
            {outlineMode === 'special' && (
              <div className="p-4 flex flex-col gap-2">
                <label className="text-xs text-gray-600">Título del discurso especial</label>
                <input
                  type="text"
                  className="border border-gray-300 rounded px-2 py-1 text-sm"
        ...
        <div className="flex justify-end gap-2 px-4 py-3 border-t border-gray-200 bg-gray-50 rounded-b-lg">
          <button
            onClick={onClose}
            className="px-4 py-1.5 text-sm border border-gray-300 rounded text-gray-600 hover:bg-gray-100"
          >
            ✕ Cancelar
          </button>
          <button
