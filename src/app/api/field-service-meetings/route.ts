...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        conductor:users!field_service_meetings_conductor_id_fkey(id, first_name, last_name, name),
        zoom_host:users!field_service_meetings_zoom_host_id_fkey(id, first_name, last_name, name),
        group:field_service_groups!field_service_meetings_group_id_fkey(id, name)
      `)
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        time_period: body.time_period || 'am',
