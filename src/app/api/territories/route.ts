...
function isSchemaMissing(error: { code?: string; message?: string } | null): boolean {
  ...
  return (
    ...
    (typeof error.message === 'string' &&
      (error.message.includes('does not exist') || error.message.includes('relation') || error.message.includes('schema cache')))
  );
}
  ...
  try {
    ...
    if (error) {
      if (isSchemaMissing(error)) {
        return NextResponse.json({ territories: [], migration_applied: false });
      }
      throw error;
    }

    // Adjuntar nombre del asignado
    ...
    const territories = (data || []).map((t: any) => ({
      ...t,
      assigned_name: t.assigned_to ? namesById[t.assigned_to] || null : null,
    }));
    ...
    if (error) {
      if (isSchemaMissing(error)) {
        return NextResponse.json(
          { error: 'Tabla territories no existe. Ejecuta sql/territories_schema.sql en Supabase.', migration_applied: false },
          { status: 409 },
        );
      }
      throw error;
