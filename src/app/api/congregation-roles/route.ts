...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from('congregation_roles')
      .select(`role_key, label, custom_label, updated_at,
        person:person_id(${USER_FIELDS}),
        assistant_1:assistant_1_id(${USER_FIELDS}),
        assistant_2:assistant_2_id(${USER_FIELDS})`);
...
export async function PUT(request: Request) {
  try {
    ...
    const { error } = await supabase.from('congregation_roles').update(patch).eq('role_key', role_key);
    if (error) throw error;
