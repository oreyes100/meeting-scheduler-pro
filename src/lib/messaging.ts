/**
 * Messaging for the territory module (Supabase/Vercel).
 *
...
export async function getMessagingSettings(congregationId: string | null): Promise<MessagingSettings> {
  const scoped = sb().from('messaging_settings').select('*');
  const { data } = congregationId
    ? await scoped.eq('congregation_id', congregationId).maybeSingle()
    : await scoped.is('congregation_id', null).maybeSingle();

  const row = (data || undefined) as Partial<MessagingSettings> | undefined;
  return {
...
export async function notify(input: NotifyInput): Promise<{ id: string; whatsapp: string } | null> {
  ...
  const settings = await getMessagingSettings(input.congregationId);
  let user: { name: string | null; phone1: string | null; phone2: string | null } | undefined;
  if (input.userId) {
    const { data } = await sb()
      .from('users')
      ...
      .eq('id', input.userId)
      .maybeSingle();
    user = (data || undefined) as typeof user;
  }
